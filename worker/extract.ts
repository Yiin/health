// The 'extracting' stage for lab_report documents: PDF bytes → text → Kimi
// structured extraction → validated LabExtraction, cached as the
// raw_extractions('extracting') payload for the normalizing stage.
//
// Text comes from unpdf (serverless pdfjs wrapper) — text-first by design;
// when the text layer is implausibly thin the PDF is scanned and the document
// halts in needs_review with reason 'scanned' (the vision path is a follow-up
// issue). Non-PDF text files are read directly.
//
...
//
// On success the stage persists documents.extracted_text (search tsvector),
// document_date and provider, and returns a payload carrying the validated
...
import { getOriginalBytes } from "../src/lib/storage.ts";
...
/**
 * Builds the extracting stage runner. Injectable seams (readBytes,
 * extractPdfText, chatStructured) keep tests free of MinIO/Kimi; production
...
export function createExtractStage(deps: ExtractStageDeps): StageRunner {
  ...
  return async (ctx) => {
    ...
    if (!document) {
      throw new Error(`document ${ctx.documentId} vanished mid-extract`);
    }

    // Other document types keep their stub behavior until their own
    // extraction stages land — the pipeline proceeds without touching data.
    if (document.document_type !== "lab_report") {
      return {
        skipped: true,
        documentType: document.document_type,
        reason: "extract stage handles lab_report documents only",
      };
    }
